...
const ToDoList: React.FC = () => {
  ...
  return (
    <>
      {showList ?
       <> < div className='toDoList' >{todoList ? todoList.map(todo =>
            <ToDoItem key={todo.id} data={todo} onEdit={handleEdit} />): null}</div > 
            <button className="add" onClick={handleAddItem}>+ New Tusk</button></> 
            : <AddEditItem editData={editData} />}

      <div className='trashTitle'><h4>Trash</h4> 
      <div className='trash'><FolderDeleteIcon fontSize='large' style={{ fill: " #af7eeb" }} />
      <span className='trashNumber'>{trash ? trash.length : null}</span>
      </div>
      </div>
      {trash.map(trashItem => (
